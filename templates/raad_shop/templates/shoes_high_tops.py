...
# Найти элементы с классом disabled-style
disabled_elements = soup.find_all('label', class_='disabled-style')

# Извлечь информацию о размерах
w_sizes = []
for label in disabled_elements:
    span_element = label.find('span')
    if span_element:
        size_info = span_element.text.strip()
        w_sizes.append(size_info)

# Вывести найденные размеры
print("Sizes with disabled JS:")
for w_size in sizes:
    print(w_size)
...
with open('../done_csv/done_shoes_high_tops.csv', mode='w', newline='', encoding='utf-8') as file:
    ...
    writer.writerow(
        [
            'Title', 'Current Price', 'Category', 'Style', 'Color', 'Mans Sizes', 'Womens Size', 'Description', 'ID'
        ]
    )
    writer.writerow([title, current_price, category, style, color_sh, ' | '.join(sizes), ' | '.join(w_sizes),
                     text if text else 'No description available', option_id])
