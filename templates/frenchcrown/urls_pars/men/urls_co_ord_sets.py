...
base_url = "https://frenchcrown.in/collections/co-ord-sets"
...
def main(base_url):
    ...
    with open('templates/../../../urls_csv/urls_co_ord_sets.csv', 'w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        writer.writerow(['url'])  # Запись заголовка
        writer.writerows([[link] for link in links])  # Запись ссылок

    print("Ссылки на товары были успешно записаны в файл 'urls_co_ord_sets.csv'.")
