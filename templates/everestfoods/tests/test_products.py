...
def get_product_info(url):
    ...
    photo_urls = ', '.join(photos[:1])  # Берем только первую фотографию
    ...
    category = 'MAN'  # Замените на актуальную категорию, если это необходимо
    ...
    if short_description_element:
        ...
        short_description = "Короткое описание не найдено."
    ...
    # Находим элемент <select> для цветов (обновите ID на актуальный)
    ...
    # Находим элемент <select> для размеров (обновите ID на актуальный)
...
input_file = '../urls_csv/products_links.csv'
...
with open(output_file, 'w', newline='', encoding='utf-8') as file:
    writer = csv.DictWriter(file,
                            fieldnames=['Product ID', 'Code Product', 'Category',
                                        'Title',
                                        'Sizes', 'Colors',
                                        'Description',
                                        'short_description',
